...
from scripts.schema.user_schemas import UserRequestSchema

user_cred = APIRouter(prefix="/api")
...
@user_cred.post("/signup", status_code=status.HTTP_201_CREATED)
def signup(user_det: UserRequestSchema):
    try:
        login_handler = LoginHandler()
